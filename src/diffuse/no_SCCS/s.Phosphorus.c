...
D 2
#define Dix0(M)	impur[P].constant[M][1] /*pre exp constant with neutral V*/
#define DixE(M)	impur[P].constant[M][2] /*exp constant with neutral V*/
#define Dim0(M)	impur[P].constant[M][3] /*pre exp constant with neg V*/
#define DimE(M)	impur[P].constant[M][4] /*exp constant with neg V*/
#define Did0(M)	impur[P].constant[M][5] /*pre exp constant with neg V*/
#define DidE(M)	impur[P].constant[M][6] /*exp constant with neg V*/
#define Dvx0(M)	impur[P].constant[M][7] /*pre exp constant with neutral V*/
#define DvxE(M)	impur[P].constant[M][8] /*exp constant with neutral V*/
#define Dvm0(M)	impur[P].constant[M][9] /*pre exp constant with neg V*/
#define DvmE(M)	impur[P].constant[M][10] /*exp constant with neg V*/
#define Dvd0(M)	impur[P].constant[M][11] /*pre exp constant with neg V*/
#define DvdE(M)	impur[P].constant[M][12] /*exp constant with neg V*/
E 2
I 2
...
E 2
	...
	for(i = 0; i < nn; i++) {
		...
		if ( conc[ss][i] > Pss ) {
D 2
		    act[ss][i] = a + p * log( conc[ss][i] - b * Pss );
		    dact[ss][ss][i] = f * p / (conc[ss][i] - b * Pss);
E 2
I 2
...
D 2
    if (is_specified(param, "Dix.0")) Dix0(mat)  = get_float( param, "Dix.0");
    if (is_specified(param, "Dix.E")) DixE(mat)  = get_float( param, "Dix.E");
    if (is_specified(param, "Dim.0")) Dim0(mat)  = get_float( param, "Dim.0");
    if (is_specified(param, "Dim.E")) DimE(mat)  = get_float( param, "Dim.E");
    if (is_specified(param, "Dimm.0")) Did0(mat)  = get_float( param, "Dimm.0");
    if (is_specified(param, "Dimm.E")) DidE(mat)  = get_float( param, "Dimm.E");
    if (is_specified(param, "Dvx.0")) Dvx0(mat)  = get_float( param, "Dvx.0");
    if (is_specified(param, "Dvx.E")) DvxE(mat)  = get_float( param, "Dvx.E");
    if (is_specified(param, "Dvm.0")) Dvm0(mat)  = get_float( param, "Dvm.0");
    if (is_specified(param, "Dvm.E")) DvmE(mat)  = get_float( param, "Dvm.E");
    if (is_specified(param, "Dvmm.0")) Dvd0(mat)  = get_float( param, "Dvmm.0");
    if (is_specified(param, "Dvmm.E")) DvdE(mat)  = get_float( param, "Dvmm.E");
E 2
I 2
    i = 0; j = 1;
