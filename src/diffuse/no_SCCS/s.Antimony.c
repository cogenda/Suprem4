h36293
s 00054/00030/00341
d D 2.1 93/02/03 14:46:44 suprem45 2 1
...
D 2
#define Dix0(M)	impur[Sb].constant[M][1] /*pre exp constant with neutral V*/
#define DixE(M)	impur[Sb].constant[M][2] /*exp constant with neutral V*/
#define Dim0(M)	impur[Sb].constant[M][3] /*pre exp constant with neg V*/
#define DimE(M)	impur[Sb].constant[M][4] /*exp constant with neg V*/
#define Fi(M)	impur[Sb].constant[M][5] /*fractional interstitialcy*/
E 2
I 2
...
E 2
	...
	for(i = 0; i < nn; i++) {
		...
		if ( chem[ss][i] > Sbss ) {
D 2
		    act[ss][i] = a + p * log( chem[ss][i] - b * Sbss );
		    dact[ss][ss][i] = f * p / (chem[ss][i] - b * Sbss);
E 2
I 2
