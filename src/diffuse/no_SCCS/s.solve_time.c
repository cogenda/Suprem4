h47148
s 00018/00013/00557
d D 2.1 93/02/03 14:47:00 suprem45 2 1
...
D 2
	switch( soltoimp[j] ) {
	case Pa : 
	    for( i = 0; i < nn; i++) nd[i]->sol[j] = olda[imptosol[P]][i];
	    break;
	case Ba : 
	    for( i = 0; i < nn; i++) nd[i]->sol[j] = olda[imptosol[B]][i];
	    break;
	case Sba : 
	    for( i = 0; i < nn; i++) nd[i]->sol[j] = olda[imptosol[Sb]][i];
	    break;
	case Asa : 
	    for( i = 0; i < nn; i++) nd[i]->sol[j] = olda[imptosol[As]][i];
	    break;
E 2
I 2
	switch( (i = soltoimp[j]) ) {
