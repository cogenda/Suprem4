...
e
s 00239/00000/00000
d D 1.1 93/02/03 14:41:17 suprem45 1 0
c date and time created 93/02/03 14:41:17 by suprem45
e
u
U
f e 0
...
D 2
#define D0(M)	impur[Cs].constant[M][1] /*pre exp constant with neutral V*/
#define DE(M)	impur[Cs].constant[M][2] /*exp constant with neutral V*/
E 2
I 2
...
D 2
    if (is_specified(param, "D.0")) D0(mat)  = get_float( param, "D.0");
    if (is_specified(param, "D.E")) DE(mat)  = get_float( param, "D.E");
    if (is_specified(param, "mobile")) {
	if ( get_bool( param, "mobile" ) )
	    impur[Cs].flags = impur[Cs].flags | MOBILE;
	else 
	    impur[Cs].flags = impur[Cs].flags & ! MOBILE;
E 2
I 2
