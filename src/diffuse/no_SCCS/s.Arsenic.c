h02758
s 00058/00029/00443
d D 2.1 93/02/03 14:46:45 suprem45 2 1
...
D 2
#define Dix0(M)	impur[As].constant[M][1] /*pre exp constant with neutral V*/
#define DixE(M)	impur[As].constant[M][2] /*exp constant with neutral V*/
#define Dim0(M)	impur[As].constant[M][3] /*pre exp constant with pos V*/
#define DimE(M)	impur[As].constant[M][4] /*exp constant with pos V*/
#define Fi(M)	impur[As].constant[M][5] /*fractional interstitialcy*/
#define Ctn0(M)	impur[As].constant[M][6] /*pre exponential part of clustering*/
#define CtnE(M)	impur[As].constant[M][7] /*activiation energy of clustering*/
#define CtnF(M)	impur[As].constant[M][8] /*clustering power*/
E 2
I 2
...
D 2
    ...
    if (is_specified(param, "Fi"))    Fi(mat)    = get_float( param, "Fi");
    if (is_specified(param, "Ctn.0")) Ctn0(mat)  = get_float( param, "Ctn.0");
    if (is_specified(param, "Ctn.E")) CtnE(mat)  = get_float( param, "Ctn.E");
    if (is_specified(param, "Ctn.F")) CtnF(mat)  = get_float( param, "Ctn.F");
E 2
I 2
    i = 0; j = 1;
