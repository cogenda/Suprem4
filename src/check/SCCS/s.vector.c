h13376
s 00001/00001/00669
d D 2.2 93/04/23 12:36:31 suprem45 3 2
c changed silicon imp refs to isilicon
e
s 00114/00026/00556
d D 2.1 93/02/03 14:12:15 suprem45 2 1
c Initial IV.GS release
c *** CHANGED *** 93/02/03 14:15:33 suprem45
c 
e
s 00582/00000/00000
d D 1.1 93/02/03 14:08:38 suprem45 1 0
c date and time created 93/02/03 14:08:38 by suprem45
e
u
U
f e 0
...
D 2
	get_Cstar( I, nn, last_temp, n, v, e );
E 2
I 2
...
E 2
	...
	for(i=0; i<nn; i++) n[i] = exp(sol_nd(i,index) / Vt);
D 2
	get_Cstar( V, nn, last_temp, n, v, e );
E 2
I 2
...
D 2
	if ( (index = imptosol[Asa]) != -1 ) 
	    for(i = 0; i < nn; i++) val[i] += sol_nd(i,index);
	if ( (index = imptosol[Sba]) != -1 ) 
	    for(i = 0; i < nn; i++) val[i] += sol_nd(i,index);
	if ( (index = imptosol[Pa]) != -1 ) 
	    for(i = 0; i < nn; i++) val[i] += sol_nd(i,index);
	if ( (index = imptosol[Ba]) != -1 ) 
	    for(i = 0; i < nn; i++) val[i] -= sol_nd(i,index);
E 2
I 2
	for ( j = 0; j < n_imp; j++ ) {
